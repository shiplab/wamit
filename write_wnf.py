...
NBODY = analysis_wamit.verify_NBODY()
...
for ii in range(NBODY):
    ...
    # Reading RAO
    [rao,rao_phase,per,inc,dof,arq4d] = analysis_wamit.raos(0)
    
    # Reading Wave Forces
    [wforce,wforce_phase,arq2d] = analysis_wamit.wave_forces(0)
        
    # Reading Drift forces (Momentum)
    [wdforce,wdforce_phase,arq8d] = analysis_wamit.drift_forces_momentum(0)
    
    # Reading Added Mass and Potential Damping
    [added_mass,pot_damp,dof1,arq1d,added_mass_matrix,pot_damp_matrix]= analysis_wamit.added_mass_pot_damping(0)
   
    # Write parameters in ship.wnf
    nome_wnf = 'ship{:d}.wnf'.format(ii)
    wnf = open(nome_wnf,'w')
    wnf.write('%OUT_FILE\n')
    wnf.write('"'+nome_out+'"\n\n')
    ...
    wnf.write('{:.1f}'.format(mass[0][0][0])+'\n\n')
